...
export async function sendEmails() {
    ...
    if (!users || users.length === 0)
        return;
    // Load HTML template
    const templatePath = path.resolve(__dirname, './assets/temp/email-temp/email-temp.html');
    const templateSource = fs.readFileSync(templatePath, 'utf-8');
    const template = Handlebars.compile(templateSource);
    // Load CSS file
    const cssPath = path.resolve(__dirname, './assets/temp/email-temp/index.css');
    const css = fs.readFileSync(cssPath, 'utf-8');
    // Process each user
    for (const user of users) {
        if (!user.preferred_retailers?.length)
            continue;
        ...
        if (retailerError || !retailerIds?.length)
            continue;
        ...
        deals.forEach(d => {
            ...
            if (!grouped[retailerName])
                grouped[retailerName] = [];
        ...
        try {
            await resend.emails.send({
                ...
                html: finalHtml,
                text: 'Your weekly Prox deals are ready. Please view this email in an HTML-capable email client.'
            });
